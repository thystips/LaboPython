...
def invert_bit_n(binary,index):
    return binary[:index] + invert_bit(binary[index]) + binary[index+1:]
...
def result_nb(binary):
    ones = count_ones(binary)
    if ones % 2 == 0:
        new_binary = invert_bit_n(binary, len(binary)-1)
    else:
        last_one_index = index_of_last_one(binary)
        new_binary = invert_bit_n(binary, last_one_index-1)
    return new_binary

def result(binary, steps):
    result = []
    last_binary = binary
    step = 0
    while step < steps:
        result.append(last_binary)
        last_binary = result_nb(last_binary)
        step += 1
    return result
